...
function PrivateChat() {
      ...
      if (userID) {
        try {
          const response = await axios.get(
            "https://hchaichi-bmqq.vercel.app/chat/users/assistance",
            { params: { recieverId: userID }, withCredentials: true }
          );
          ...
          if (firstUser) {
            setChatUser({
              ...
              username:
                firstUser.firstName +
                (firstUser.lastName !== "its$fromgoogle"
                  ? " " + firstUser.lastName
                  : ""),
              role: firstUser.role,
              image:
          ...
          const promises = response.data.map((user) =>
            axios.get(
              `https://hchaichi-bmqq.vercel.app/check_user_status/${user._id}`,
              { withCredentials: true }
            )
          );
  ...
  const handleUserClick = (user) => {
    setChatUser({
      ...
      role: user.role,
      image:
        user.image || user.imageUrl || "/assets/images/Default-profile.jpg",
        ...
        const checkUserStatus = async () => {
            try {
                const promises = users.map(user =>
                    axios.get(`https://hchaichi-bmqq.vercel.app/check_user_status/${user._id}`, { withCredentials: true })
                );
                const responses = await Promise.all(promises);
                const status = {};
            ...
            <div className="card-body pt-5" id="kt_chat_contacts_body">
              ...
              >
                {filteredUsers.map((user) => (
                  <>
                    <div
                      key={"user-box" + user._id}
                      className="d-flex flex-stack py-4 pointer"
                          ...
                          <div
                            className={`symbol-badge ${
                              isUserConnected(user._id)
                                ? "bg-success"
                                : "bg-secondary"
                            } start-100 top-100 border-4 h-8px w-8px ms-n2 mt-n2`}
                          ></div>
                        </div>
                        <div className="ms-5">
