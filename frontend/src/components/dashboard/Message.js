...
const Message = ({ message, onEdit, onDelete }) => {
  ...
  const handleEdit = async () => {
    try {
      const response = await axios.put(
        `https://hchaichi-bmqq.vercel.app/chat/user/edit/${message._id}`,
        { content },
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(
        `https://hchaichi-bmqq.vercel.app/chat/user/delete/${message._id}`,
        { withCredentials: true }
      );
        ...
        {!isEditing && !message.isDeleted && (
          <div className="card-toolbar">
            <div className="me-n3" style={{ position: "relative" }}>
              <>
                <button
                  className="btn btn-sm btn-icon me-1 show"
                  data-kt-menu-trigger="click"
                  data-kt-menu-placement="bottom-end"
                  onClick={() => toggleDropDown("menu" + message._id)}
                >
                ...
                <div
                  key={"menu" + message._id}
                  className={`menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-semibold w-200px py-3 ${
                    isVisible === "menu" + message._id ? "show" : ""
                  }`}
                  data-kt-menu="true"
                  style={filterStyle}
                >
