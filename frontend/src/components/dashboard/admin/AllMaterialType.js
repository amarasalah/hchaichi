...
function AllMaterialType() {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(
        `https://hchaichi-bmqq.vercel.app/product/materialtypes`,
        {
          params: {
            page: currentPage,
            limit: PerPage,
            ...
            <div className="symbol rounded-0 symbol-50px overflow-hidden me-3">
              <a href="#">
                <div className="symbol-label">
                  <img
                    src={
                      `https://hchaichi-bmqq.vercel.app/product/image/materialtype/${row.original.avatarMaterialType}` ||
                      "/assets/images/Default-profile.jpg"
                    }
                    alt={row.original.materialType_name}
                    className="w-100"
                  />
                </div>
              </a>
...

      {
        ...
        Cell: ({ row }) => (
          <div className="text-end">
            <a
              href={`/admin/updateMaterialType/${row.original._id}`}
  ...
  const handleDelete = async (userId) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(
          `https://hchaichi-bmqq.vercel.app/product/deletematerialtype/${userId}`,
          { withCredentials: true }
        );
        ...
        Swal.fire(
          "Error!",
          "There was an error deleting the material Type.",
          "error"
        );
      }
    }
