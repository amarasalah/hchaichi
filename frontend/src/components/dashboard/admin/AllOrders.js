...
function AllOrders() {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(
        `https://hchaichi-bmqq.vercel.app/product/allorders`,
        {
          params: {
            page: currentPage,
            limit: PerPage,
  ...
  const handleAction = async (orderId, action) => {
    try {
      ...
      const response = await axios.post(
        `https://hchaichi-bmqq.vercel.app/product/${action}order`,
        { id: orderId },
        { withCredentials: true }
      );
      Swal.close();
      ...
      Swal.close();
      console.error(
        `Error ${action === "approve" ? "approving" : "declining"} order:`,
        error
      );
      Swal.fire(
        "Error",
    ...
    () => [
      {
        ...
        Cell: ({ row }) => (
          <a
            href={`/admin/orderView/${row.original._id}/${row.original.userId}`}
            className="btn btn-link btn-color-dark btn-active-color-primary me-5 mb-2"
          >
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <span>
            ...
            {row.original.lastName !== "its$fromgoogle"
      ...
      },
      {
        ...
        Cell: ({ row }) => {
          ...
          const totalAnglesCost = angles.reduce(
            (total, angle) => total + angle.OriginalAngleCost,
            0
          );
          const totalEdgesCost = edges.reduce(
          ...
          const totalVatAngles = angles.reduce(
            (total, angle) =>
              total + angle.OriginalAngleCost * 0.01 * angle.VatAngle,
            0
          );
          const totalVatEdges = edges.reduce(
            (total, edge) =>
              total + edge.OriginalEdgeCost * 0.01 * edge.VatEdge,
            0
          );
          const totalDiscount =
            DiscountShapeDT +
            DiscountMaterialDT +
            angles.reduce((total, angle) => total + angle.DiscountAngleDT, 0) +
            edges.reduce((total, edge) => total + edge.DiscountEdgeDT, 0);
          ...
          <div className="text-center">
            <div className="d-flex">
              {row.original.status === "pending" ? (
                <>
                  <button
                    className="btn btn-icon btn-success rounded-0 me-2"
                    onClick={() => handleAction(row.original._id, "approve")}
                  >
                    <i className="ki-duotone ki-double-check fs-1">
                      {" "}
                      <span className="path1"></span>{" "}
                      <span className="path2"></span>{" "}
                    </i>
                  ...
                  <button
                    className="btn btn-icon btn-warning rounded-0 me-2"
                    onClick={() => handleAction(row.original._id, "decline")}
                  >
              ...
              <button
                className="btn btn-icon btn-danger rounded-0 me-2"
              ...
              <a
                href={`/admin/editorder/${row.original._id}/`}
                className="btn btn-icon btn-info rounded-0 me-2"
              >
  ...
  const handleDelete = async (orderId) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(
          `https://hchaichi-bmqq.vercel.app/product/deleteorder/${orderId}`,
          { withCredentials: true }
        );
            ...
            >
              <button
                ...
                className={`btn btn-light-primary me-3 ${
                  isFilterVisible ? "show menu-dropdown" : ""
                }`}
              ...
              <div
                ...
                data-kt-menu="true"
                style={isFilterVisible ? filterStyle : {}}
              >
            ...
            <div className="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
              <div className="dt-paging paging_simple_numbers">
                <ul className="pagination">
                  ...
                  >
                    <a className="page-link next" tabIndex="0">
                      <i
                        className="next"
                        onClick={() => handlePageChange(currentPage + 1)}
                      ></i>
