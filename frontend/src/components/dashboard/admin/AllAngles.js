...
function AllAngles() {
  ...
  const getDiscountType = (discountOption) => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(
        `https://hchaichi-bmqq.vercel.app/product/angles`,
        {
          params: {
            page: currentPage,
            limit: PerPage,
  ...
  const columns = useMemo(
    () => [
            ...
            <div className="symbol rounded-0 symbol-50px overflow-hidden me-3">
              <a href="#">
                <div className="symbol-label">
                  <img
                    src={
                      `https://hchaichi-bmqq.vercel.app/product/image/angle/${row.original.avatarAngle}` ||
                      "/assets/images/Default-profile.jpg"
                    }
                    alt={row.original.Angle_name}
                    className="w-100"
                  />
                </div>
              </a>
      ...
      },
      {
        ...
        Cell: ({ row }) => getDiscountType(row.original.discount_option),
      },
      {
        Header: "Discount",
        Cell: ({ row }) =>
          getDiscountValue(
            row.original.discount_option,
            row.original.discounted_price,
            row.original.discounted_percentage
          ),
      },
      {
        Header: "VAT Amount (%)",
...

      {
        ...
        Cell: ({ row }) => (
          <div className="text-end">
            <a
              href={`/admin/updateangle/${row.original._id}`}
              className="btn btn-icon btn-warning rounded-0 me-2"
            >
              <i className="ki-duotone ki-pencil fs-1">
                {" "}
  ...
  const handleDelete = async (userId) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(
          `https://hchaichi-bmqq.vercel.app/product/deleteangle/${userId}`,
          { withCredentials: true }
        );
        ...
        Swal.fire("Error!", "There was an error deleting the angle.", "error");
      }
    }
  };
