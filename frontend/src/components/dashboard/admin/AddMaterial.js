...
function AddMaterial() {
  ...
  const [materialTypes, setMaterialTypes] = useState([]); // State to hold material types
  const [formData, setFormData] = useState({
    material_name: "",
    materialdescription: "",
    thickness: [""], // Initialize with one empty string for the first input field
    price: "",
    discount_option: "1",
  ...
  const handleSubmit = async (e) => {
    ...
    const requiredFields = ["avatarMaterial", "material_name", "price"];
    ...
    try {
      const response = await axios.post(
        "https://hchaichi-bmqq.vercel.app/product/addmaterial",
        data,
        {
          headers: {
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "There was an error adding the material!",
      });
    }
    btnRef2.current.style.display = "none";
    btnRef1.current.style.display = "block";
