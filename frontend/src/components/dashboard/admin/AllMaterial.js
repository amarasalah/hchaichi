...
function AllMaterial() {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(
        `https://hchaichi-bmqq.vercel.app/product/materials`,
        {
          params: {
            page: currentPage,
            limit: PerPage,
  ...
  const columns = useMemo(
    () => [
            ...
            <div className="symbol rounded-0 symbol-50px overflow-hidden me-3">
              <a href="#">
                <div className="symbol-label">
                  <img
                    src={
                      `https://hchaichi-bmqq.vercel.app/product/image/material/${row.original.avatarMaterial}` ||
                      "/assets/images/Default-profile.jpg"
                    }
                    alt={row.original.material_name}
                    className="w-100"
                  />
                </div>
              </a>
      ...
      {
        Header: "Thickness",
        Cell: ({ row }) => row.original.thickness.join(", "),
      },
      {
        Header: "Status",
        Cell: ({ row }) =>
...

      {
        ...
        Cell: ({ row }) => (
          <div className="text-end">
            <a
              href={`/admin/updatematerial/${row.original._id}`}
  ...
  const handleDelete = async (userId) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(
          `https://hchaichi-bmqq.vercel.app/product/deletematerial/${userId}`,
          { withCredentials: true }
        );
