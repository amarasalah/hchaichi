...
function AllShapes() {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(
        `https://hchaichi-bmqq.vercel.app/product/shapes`,
        {
          params: {
            page: currentPage,
            limit: PerPage,
  ...
  const columns = useMemo(
    () => [
            ...
            <div className="symbol rounded-0 symbol-50px overflow-hidden me-3">
              <a href="#">
                <div className="symbol-label">
                  <img
                    src={
                      `https://hchaichi-bmqq.vercel.app/product/image/shape/${row.original.avatarShapeImg}` ||
      ...
      },
      {
        ...
        Cell: ({ row }) => getDiscountType(row.original.discountOption),
      },
      {
        Header: "Discount",
        Cell: ({ row }) =>
          getDiscountValue(
            row.original.discountOption,
            row.original.fixedDiscount,
            row.original.pourcentageDiscount
          ),
      },
      {
        Header: "VAT Amount (%)",
...

      {
        ...
        Cell: ({ row }) => (
          <div className="text-end">
            <a
              href={`/admin/updateshape/${row.original._id}`}
  ...
  const handleDelete = async (userId) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(
          `https://hchaichi-bmqq.vercel.app/product/deleteshape/${userId}`,
          { withCredentials: true }
        );
        ...
        Swal.fire("Error!", "There was an error deleting the shape.", "error");
      }
    }
  };
