...
function UpdateMaterial() {
    ...
    const fetchMaterial = async () => {
      try {
        const response = await axios.get(
          `https://hchaichi-bmqq.vercel.app/product/material/${MaterialId}`,
          { withCredentials: true }
        );
        const pivot = {
          material_name: response.data.material.material_name,
          materialdescription: response.data.material.materialdescription,
          thickness: response.data.material.thickness,
          price: response.data.material.price,
          discount_option: "" + response.data.material.discount_option,
          discounted_percentage: response.data.material.discounted_percentage,
          discounted_price: response.data.material.discounted_price,
          vat_amount: response.data.material.vat_amount,
          status: response.data.material.status,
          avatarMaterial: null,
          materialtype: response.data.material.materialType,
        ...
        if (response.data.material.avatarMaterial) {
          setImagePreviews({
            avatarMaterial:
              "https://hchaichi-bmqq.vercel.app/product/image/material/" +
              response.data.material.avatarMaterial,
          });
        } else {
          setImagePreviews({
            avatarMaterial: "/assets/media/svg/files/blank-image.svg",
          });
        }
      } catch (err) {
        console.log("There was an error fetching the material!");
      }
    };
  ...
  useEffect(() => {
    ...
    setImagePreviews((prev) => ({
      ...prev,
      avatarMaterial: formData.avatarMaterial
        ? URL.createObjectURL(formData.avatarMaterial)
        : prev.avatarMaterial,
    }));
  ...
  useEffect(() => {
    ...
    axios
      .get("https://hchaichi-bmqq.vercel.app/product/order/materialtypes")
      .then((response) => {
        setMaterialTypes(response.data.materialtypes);
      })
      .catch((error) => {
        console.error("Error fetching material types:", error);
        Swal.fire("Error", "Could not fetch material types", "error");
      });
  }, []);
  ...
  const handleMTChange = (selectedOption) => {
    setFormData({
      ...formData,
      materialtype: selectedOption.value,
      material: selectedOption.dataM,
    });
  };
  ...
  const handleVariationChange = (index, value) => {
    if (value > 0) {
      ...
      Swal.fire({
        icon: "error",
        title: "Validation Error",
        text: `Thickness Variations must be more than 0.`,
      });
    }
  };

  const addVariation = () => {
    setFormData({ ...formData, thickness: [...formData.thickness, ""] });
  };
  const removeVariation = (index) => {
    const newThickness = formData.thickness.filter((_, i) => i !== index);
    setFormData({ ...formData, thickness: newThickness });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const requiredFields = ["material_name", "price"];
    ...
    if (selectedOption === "2") {
      if (
        !formData.discounted_percentage ||
        formData.discounted_percentage <= 0 ||
        formData.discounted_percentage > 100
      ) {
        Swal.fire({
          icon: "error",
          title: "Validation Error",
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "Material Updating Successfully",
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "There was an error updating the Material!",
      });
    }
    btnRef2.current.style.display = "none";
    btnRef1.current.style.display = "block";
  ...
  const options = materialTypes.map((type) => ({
    value: type.materialType_name,
    dataM: type.material,
    label: (
      <span>
        <strong>{type.materialType_name}</strong> <em>({type.material})</em>
      </span>
    ),
  }));
            ...
            <div className="card-body text-center pt-0">
              ...
              >
                <div
                  ...
                  style={{
                    backgroundImage: `url(${imagePreviews.avatarMaterial})`,
                  }}
                ></div>
              ...
              <div className="text-muted fs-7">
                Set the material thumbnail image. Only *.png, *.jpg, *.svg and
            ...
            >
              <div className="d-flex flex-column gap-7 gap-lg-10">
                  ...
                  <div className="card-body pt-0">
                    <div className="mb-2 fv-row">
                      ...
                      <input
                        ...
                        defaultValue={formData.material_name}
                        onChange={handleInputChange}
                        required
                      />

                      <div className="text-muted fs-7">
                        A material name is required and recommended to be
                        unique.
                      </div>
                    </div>
                    ...
                    <div className="mb-2 fv-row">
                      ...
                      <div className="mb-2">
                        <textarea
                          ...
                          defaultValue={formData.materialdescription}
                    ...
                    <div className="mb-2 fv-row">
                      ...
                      <div className="text-muted fs-7">
                        Assign a material type from your material.
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="card card-flush py-4">
                  <div className="card-header">
                    <div className="card-title">
                      <h2>Add Thickness Variations (mm)</h2>
                    </div>
                  </div>
                  <div className="card-body pt-0">
                    <div
                      ...
                      <div id="kt_ecommerce_add_product_options">
                        <div className="form-group">
                          <div className="d-flex flex-column gap-3">
                            {formData.thickness.map((variation, index) => (
                              <div
                                key={index}
