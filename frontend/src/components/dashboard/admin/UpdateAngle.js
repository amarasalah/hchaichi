...
function UpdateAngle() {
    ...
    const fetchAngle = async () => {
      try {
        const response = await axios.get(
          `https://hchaichi-bmqq.vercel.app/product/angle/${AngleId}`,
          { withCredentials: true }
        );
        const pivot = {
          angle_name: response.data.angle.Angle_name,
          angledescription: response.data.angle.Angledescription,
          border_radius: response.data.angle.border_radius,
          price: response.data.angle.price,
          discount_option: "" + response.data.angle.discount_option,
          discounted_percentage: response.data.angle.discounted_percentage,
          discounted_price: response.data.angle.discounted_price,
          vat_amount: response.data.angle.vat_amount,
          status: response.data.angle.status,
          avatarAngle: null,
        };
        setFormData(pivot);
        ...
        if (response.data.angle.avatarAngle) {
          setImagePreviews({
            avatarAngle:
              "https://hchaichi-bmqq.vercel.app/product/image/angle/" +
              response.data.angle.avatarAngle,
          });
        } else {
          setImagePreviews({
        ...
        console.log("There was an error fetching the angle!");
      }
    };
  ...
  useEffect(() => {
    ...
    setImagePreviews((prev) => ({
      ...prev,
      avatarAngle: formData.avatarAngle
        ? URL.createObjectURL(formData.avatarAngle)
        : prev.avatarAngle,
    }));
  ...
  const removeVariation = (index) => {
    const newBorderRadius = formData.border_radius.filter(
      (_, i) => i !== index
    );
    setFormData({ ...formData, border_radius: newBorderRadius });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const requiredFields = ["angle_name", "price"];
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "Angle Updating Successfully",
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "There was an error updating the Angle!",
      });
    }
    btnRef2.current.style.display = "none";
    btnRef1.current.style.display = "block";
            ...
            <div className="card-body text-center pt-0">
              ...
              >
                <div
                  ...
                  style={{
                    backgroundImage: `url(${imagePreviews.avatarAngle})`,
                  }}
                ></div>
              ...
              <div className="text-muted fs-7">
                Set the angle thumbnail image. Only *.png, *.jpg, *.svg and
                  ...
                  <div className="card-body pt-0">
                    <div className="mb-2 fv-row">
                      ...
                      <input
                        ...
                        defaultValue={formData.angle_name}
                        onChange={handleInputChange}
                        required
                      />

                      <div className="text-muted fs-7">
                        A angle name is required and recommended to be unique.
                      </div>
                    </div>
                    ...
                    <div className="mb-2 fv-row">
                      ...
                      <div className="mb-2">
                        <textarea
                          ...
                          defaultValue={formData.angledescription}
